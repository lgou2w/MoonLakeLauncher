...
class MuiResourceImage: MuiStackPane {
    ...
    constructor(): super() {
        this.url.addListener { _, _, newValue -> run {
            if(newValue != null) {
                val stream = ClassUtils.loadResourceAsInputStream(MuiResourceImage::class.java, newValue)
                this.image.image = Image(stream)
            }
        }}
        this.children.add(0, image)
        this.image.styleClass.add("mui-res-image")
    }
